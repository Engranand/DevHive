...
export default function GitHubPage() {
  ...
  if (loading) return <Layout><div className="p-6 text-muted text-sm font-mono">Loading GitHub data...</div></Layout>
  if (!data) return <Layout><div className="p-6 text-muted text-sm font-mono">Failed to load GitHub data.</div></Layout>
    ...
    <Layout>
      <div className="p-6 space-y-5">
        ...
        <div className="flex items-center justify-between">
          <div>
            <div className="text-xs text-muted font-mono uppercase tracking-widest mb-1">// GitHub Integration</div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-surface border border-border rounded-xl p-4">
            ...
            <div className="space-y-2">
              {pullRequests.length === 0 && <div className="text-sm text-muted">No pull requests yet.</div>}
              {pullRequests.map(pr => (
        ...
        <div className="bg-surface border border-purple/20 rounded-xl p-4">
          <div className="text-xs font-mono text-purple uppercase tracking-wider mb-3">// Automation Impact</div>
          ...
          <div className="mt-3 pt-3 border-t border-border text-xs text-muted">
            When a linked PR merges, DevHive automatically moves the task to <span className="text-success">Done</span> and broadcasts via Socket.io — no manual action needed.
          </div>
        </div>
